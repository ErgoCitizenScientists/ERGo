...
volatile uint8_t sendERGMessage=0;
      ...
      if(sendERGMessage)
      {
        sendERGMessage = 0;
          int i;
            ...
            //send message
            i = 0;
            while(eventMessage[i] != 0)
            {
                reading[head++] = eventMessage[i++];
                if(head==BUFFER_SIZE)
                {
                  head = 0;
                }
...
void sendMessage(const char * message)
{
  ...
  int i;
  int tempHead = head;
  int newHeadPosition = tempHead+31;
  if(newHeadPosition>=BUFFER_SIZE)
  {
    newHeadPosition= newHeadPosition-BUFFER_SIZE;
  }
  ...
  for(i=0;i< ESCAPE_SEQUENCE_LENGTH;i++)
  {
      reading[tempHead++] = escapeSequence[i];
      if(tempHead==BUFFER_SIZE)
      {
        tempHead = 0;
      }
  }
  ...
  for(i=0;i< ESCAPE_SEQUENCE_LENGTH;i++)
  {
      reading[tempHead++] = endOfescapeSequence[i];
      if(tempHead==BUFFER_SIZE)
      {
        tempHead = 0;
      }
